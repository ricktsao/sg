...
//  answer=title+solar.y+"年"+solar.m+"月"+solar.d+"日,星期"+solar.w;
  if(type==0) { //陽->陰
   ...
   // answer="陰曆"+lunar.y+"("+GanGB[gan.y]+ZhiGB[zhi.y]+")年"+(lunar.l?"閏":"")+lunar.m+"月"+lunar.d+"日,生肖屬"+ShengXiaoGB[zhi.y]; 
    answer="<tr><td>農曆</td><td>"+taiwan_year+"年"+lunar.m+"月"+lunar.d+"日</td></tr>"; 
...
function check_ymd(y,m,d) {
  if(y<1901 || y>2049) {
     console.log("年份只能介於 1901 至 2049 之間");
     return false;
  }
  switch(m) {
    case 2:
          if((y%4==0) && d>29) {
              console.log(m+"月只有 29天");
              return false;
          }
          else if((y%4!=0) && d>28) {
              console.log(m+"月只有 28天");
              return false;
          }
	  break;
    case 4: 
...
/*----------------------------------------------------------*/
function mainx(_yy,_mm,_dd,_type) {
//  alert("test");
  var type;
  var x=document.forms["ymd"];
  var year,month,day;

  year=parseInt(_yy,10);
  if(isNaN(year)) {
    //x.answer.value="年份只能填數字,你亂填喔..";
    return;
  }
  month=parseInt(_mm,10);
  day=parseInt(_dd,10);
